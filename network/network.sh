...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-applicant.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating verifierA Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-verifierA.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating verifierB Identities"
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.applicant.example.com applicant
    peer_cert admin Admin@applicant.example.com applicant

    infoln "Creating verifierA Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.verifierA.example.com verifierA
    peer_cert admin Admin@verifierA.example.com verifierA
  ...
  infoln "Generating CCP files for applicant and verifierA"
...
# call the script to create the channel, join the peers of applicant and verifierA,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to applicant and verifierA, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.applicant.example.com docker_peer0.verifierA.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/applicant/msp organizations/fabric-ca/applicant/tls-cert.pem organizations/fabric-ca/applicant/ca-cert.pem organizations/fabric-ca/applicant/IssuerPublicKey organizations/fabric-ca/applicant/IssuerRevocationPublicKey organizations/fabric-ca/applicant/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/verifierA/msp organizations/fabric-ca/verifierA/tls-cert.pem organizations/fabric-ca/verifierA/ca-cert.pem organizations/fabric-ca/verifierA/IssuerPublicKey organizations/fabric-ca/verifierA/IssuerRevocationPublicKey organizations/fabric-ca/verifierA/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/verifierB/msp organizations/fabric-ca/verifierB/tls-cert.pem organizations/fabric-ca/verifierB/ca-cert.pem organizations/fabric-ca/verifierB/IssuerPublicKey organizations/fabric-ca/verifierB/IssuerRevocationPublicKey organizations/fabric-ca/verifierB/fabric-ca-server.db'
